...
// Change the click listener to NOT close if you click the overlay
...
// NEW: Automatically expand every category on initial load
    state.expandedCategories = new Set(state.categories.map(c => c.id));
...
buildHeader() {
    ...
    header.innerHTML = `
      ...
      <div class="pl-header-actions">
        ...
        <button class="pl-btn" id="pl-clear-all" title="Clear all selected prompts" style="background: #444; color: white;">Reset</button>
 ...
 // New Reset Logic with Confirmation
    header.querySelector("#pl-clear-all").addEventListener("click", () => {
      // Create the content for the confirmation box
      const confirmationContent = `
        <div class="pl-modal-header">
          <h2>Reset Selections?</h2>
          <button class="pl-modal-close" id="mclose">✕</button>
        </div>
        <div class="pl-form">
          <p>Are you sure you want to clear all selected prompts? This cannot be undone.</p>
          <div class="pl-form-actions">
            <button class="pl-btn" id="f-cancel">Cancel</button>
            <button class="pl-btn pl-btn-danger" id="f-confirm">Clear All</button>
          </div>
        </div>
      `;

      // Use the built-in showModal function
      const { modal, close } = showModal(confirmationContent);

      // Close if they hit 'X' or 'Cancel'
      modal.querySelector("#mclose").addEventListener("click", close);
      modal.querySelector("#f-cancel").addEventListener("click", close);

      // Only reset if they hit the red 'Clear All' button
      modal.querySelector("#f-confirm").addEventListener("click", () => {
        ...
        if (this.node) {
          ...
          if (wIds) wIds.value = "";
          if (wTitles) wTitles.value = "";
          this.node.setDirtyCanvas(true);
		  state.selectedPromptIds = [];
        // NEW: Re-expand everything on reset
        state.expandedCategories = new Set(state.categories.map(c => c.id));
        }
...
if (hasChildren && expanded) {
      ...
      // Sort the children alphabetically before adding them to the UI
      const sortedChildren = [...children].sort((a, b) => 
        a.name.toLowerCase().localeCompare(b.name.toLowerCase())
      );

      sortedChildren.forEach((child) => 
        childWrap.appendChild(this.buildCategoryItem(child, depth + 1))
      );
...
const bar = document.createElement("div");
  ...
  togglePrompt(prompt) {
    ...
    // === KEY FIX: Preserve scroll position ===
    const contentArea = this.container.querySelector(".pl-content");
    ...
    setTimeout(() => {
      const newContentArea = this.container.querySelector(".pl-content");
      if (newContentArea) {
  ...
  // Smooth scroll preservation for both sidebar and content
  _preserveScrolls(callback) {
    const sidebar = this.container.querySelector(".pl-sidebar");
    const content = this.container.querySelector(".pl-content");
    ...
    // Restore immediately after DOM update
  ...
  openPromptEditor(existing) {
    ...
    const content = `
      ...
      <div class="pl-form">
        ...
        <textarea id="f-text" class="pl-textarea" rows="6" style="resize: vertical;">${existing?.text || ""}</textarea>
